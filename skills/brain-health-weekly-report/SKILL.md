...
이 스킬이 다루는 지표 정의와 정상/위험 값 범위, 그리고 테스트용
`assets/sample_week.json` 은 임의로 지어낸 수치가 아니라 **AI-Hub의 실제
의료 데이터셋**을 근거로 설계되었습니다.
...
지표 매핑: `sleep_efficiency_pct`·`night_awakenings_avg`·`hrv_rmssd_ms`·
`resting_hr_bpm` 은 수면 라이프로그에서, `daily_steps_avg` 는 걸음거리
라이프로그에서, `cognitive_test_score` 는 MMSE 검사 점수에서 비롯된
지표입니다. 따라서 이 스킬의 입력 스키마와 위험 판정 기준은 실제 치매
고위험군 코호트의 데이터 분포에 기반합니다.

> 참고: 원천 CSV → 입력 JSON 변환 도구는 별도 `data_pipeline` 스크립트로
> 제공되며, 진단군(정상군 CN / 경도인지장애 MCI / 치매군 Dem)별 대표
> 피험자 데이터를 재현할 수 있습니다. 개인정보 보호를 위해 원천 데이터
> 자체는 저장소에 포함하지 않습니다.
